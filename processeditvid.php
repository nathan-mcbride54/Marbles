<?php
// ------------------------------------------------------------------
// NAME: processeditvid.php
// DESCRIPTION: Processing page used to edit a video in the users library.
// -------------------------------------------------------------------
  session_start();
  ...
  if (isset($_POST['submit'])){
    ...
    $pdo = & dbconnect();
    $sql= "UPDATE marbles_Movies SET title = ?, year = ?, actors = ?, studio = ?, plotsummary = ?, genre = ?, starrating = ?, mpaarating = ?, runtime = ?, theatrerelease = ?, dvdrelease = ?, videotype = ? WHERE movieid = ?";
    $stmt = $pdo->prepare($sql);
    $stmt->execute([$formtitle,$formyear,$formactors,$formstudio,$formplot,$formgenre,$formstarrating,$formmpaarating,$formruntime,$formtheatrerelease,$formdvdrelease,$formvideotype, $movieid]);
    $target_dir = "www_data/uploads/";


    if(isset($_POST['filetarget']) && $_POST['fileToUpload'] != null){
